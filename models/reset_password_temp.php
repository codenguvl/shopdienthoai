...
class ResetPasswordTemp
{
    ...
    private function sendEmail($email, $key, $basePath)
    {
        ...
        $output .= '<p>-------------------------------------------------------------</p>';
        $output .= '<p><a href="' . $basePath . '?page=resetmatkhau&key=' . $key . '&email=' . $email . '&action=reset" target="_blank">
        ...
        $output .= '<p>Cảm ơn,</p>';
        $output .= '<p>Shopdienthoai</p>';
        $body = $output;
        $subject = "Khoi phuc mat khau - shopdienthoai.com";
